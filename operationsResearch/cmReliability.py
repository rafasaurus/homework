...
def fixed_left_edge_chord(leftLambda, __lambda__, iterations):  #fixed with left edge chord method "quasi-Newton method" 167 Simonyan
    for o in range(iterations):
        ...
        lambdaf_dict = compute_global_prob(func(index-1, C, m, __lambda__))
        print("lambda:", __lambda__ , "\ndict", lambdaf_dict) 
        # lambdaf_dict["lambda"] = round(lambdaf_dict["lambda"],5)
        lambdaf = lambdaf_dict["wm"]-W
        boolean = True
        ''' 
        if lambdaf <= (min(cost)):
            return lambdaf_dict
        '''  
        lambdaf_left_dict = compute_global_prob(func(index-1, C, m, leftLambda))
        # print(lambdaf_left_dict)
        # lambdaf_left_dict["lambda"] = round(lambdaf_dict["lambda"],5)
        lambdaf_left = lambdaf_left_dict["wm"]-W
        boolean = True
...
def func(index, C, m, __lambda__):
    ...
    global debug_index
    if index == 0:
        for i in range(int((C)/cost[index]+1)):  # W/wi
        ...
        for i in range(int((C)/cost[index]+1)):  # mj = C/cj # +1 ================================================================
            m = np.append(m, i)
...
'''
for __lambda__ in l:# np.arange(0.00001, 0.00023 , 0.000001):  # for my problem
    start_time = time.time()
    print("-----------------------------------------------------------------------")
    global_dictionary = func(index-1, C, m, __lambda__)  ######
    print("global:", compute_global_prob(global_dictionary))        
    boolean = True
    elapsed_time = time.time()-start_time
    print("time elapsed for the program in ms ", elapsed_time*1000)
...
fixed_left_edge_chord(leftLambda = 0.00002, __lambda__ = 0.02, iterations = 10)
'''
